...
The patch fixes 13.4.1 workstation issues observed with Codex `0.137.0`:
...
- Codex `PostToolUse` rejected worker output containing top-level
  `suppressOutput`; the Codex `PostToolUse` command now pipes through
  `scripts/codex-hook-output-filter.js`.
